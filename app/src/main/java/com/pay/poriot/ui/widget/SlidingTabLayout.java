...
import com.pay.poriot.listener.OnTabSelectListener;
import com.pay.poriot.util.DensityUtils;
import com.pay.poriot.util.UnreadMsgUtils;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener{
     ...
     * text bg
     */
    private boolean mTextBgIsCircle;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextsizeSelect = ta.getDimension(R.styleable.SlidingTabLayout_tl_textsize_select, sp2px(14));
        ...
        mTabPadding = ta.getDimension(R.styleable.SlidingTabLayout_tl_tab_padding, mTabSpaceEqual || mTabWidth > 0 ? dp2px(0) : dp2px(10));


        mTextBgIsCircle = ta.getBoolean(R.styleable.SlidingTabLayout_tl_text_bg_is_circle, false);
        mTextBgColor = ta.getColor(R.styleable.SlidingTabLayout_tl_text_bg_color, Color.parseColor("#AAffffff"));
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (tvTabTitle != null) {
                ...
                if (mTextBgIsCircle) {
                    if (i == mCurrentTab) {
                        tvTabTitle.setBackground(SelectorFactory.newShapeSelector()
                                .setDefaultBgColor(mTextBgColor)
                                .setCornerRadius(DensityUtils.getMeasureValue(40))
                                .create());
                    } else {
                        tvTabTitle.setBackground(null);
                    }

                    tvTabTitle.setPadding((int) mTabPadding, 10, (int) mTabPadding, 10);
                } else {
                    tvTabTitle.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
                }


                tvTabTitle.setTextSize(TypedValue.COMPLEX_UNIT_PX, i == mCurrentTab ? mTextsizeSelect : mTextsize);
                if (mTextAllCaps) {
                ...
                if (mTextBold == TEXT_BOLD_BOTH) {
                    ...
                    tvTabTitle.getPaint().setFakeBoldText(i == mCurrentTab);
                }
            }
        }
    }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tabTitle != null) {
                ...
                tabTitle.setTextSize(TypedValue.COMPLEX_UNIT_PX, isSelect ? mTextsizeSelect : mTextsize);

                if (mTextBgIsCircle) {
                    if (isSelect) {
                        tabTitle.setBackground(SelectorFactory.newShapeSelector()
                                .setDefaultBgColor(mTextBgColor)
                                .setCornerRadius(DensityUtils.getMeasureValue(40))
                                .create());
                    } else {
    ...
    private void calcIndicatorRect() {
        ...
        if (mIndicatorStyle == STYLE_NORMAL && mIndicatorWidthEqualTitle) {
            TextView tvTitle = currentTabView.findViewById(R.id.tv_tab_title);
            mTextPaint.setTextSize(mTextsize);
            float textWidth = mTextPaint.measureText(tvTitle.getText().toString());
