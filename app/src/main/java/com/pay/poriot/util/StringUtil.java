...
//        Pattern p = Pattern.compile("^(13|14|15|16|17|18|19)\\d{9}$");
    ...
    public static String addTimeParam(String url) {
        String param;
        if (url.contains("?")) {
            param = "&time=";
        } else {
        ...
        return url.concat(param + System.currentTimeMillis());
    }


    public static String getSimpleTel(String tel) {
        String simpleTel = tel.replaceAll("(\\d{3})\\d{4}(\\d{4})", "$1****$2");
        return simpleTel;
    }

    public static boolean isStandardPwd(String pwd) {
        String PWD_PATTERN = "^(?![A-Za-z_\\W]+$)(?![0-9_\\W]+$)[a-zA-Z0-9_\\W]+$";
